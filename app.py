...
app.config["TELEGRAM_BOT_TOKEN"] = os.environ.get(
    "TELEGRAM_BOT_TOKEN",
    "8706088814:AAF9yz41489u0Jr8wyzHvwJ1IxVcrF_0A",
)
...
app.config["SEND_STARTUP_TELEGRAM_TEST"] = os.environ.get("SEND_STARTUP_TELEGRAM_TEST", "1") == "1"
...
def telegram_is_configured() -> bool:
    return bool(
        app.config.get("TELEGRAM_REMINDERS_ENABLED")
        and app.config.get("TELEGRAM_BOT_TOKEN")
        and app.config.get("TELEGRAM_CHAT_ID")
    )


def send_telegram_notification(message: str) -> bool:
    if not telegram_is_configured():
        print("TELEGRAM STATUS: disabled or not configured", flush=True)
        return False

    url = f"https://api.telegram.org/bot{app.config['TELEGRAM_BOT_TOKEN']}/sendMessage"
    payload = {
        "chat_id": str(app.config["TELEGRAM_CHAT_ID"]).strip(),
        "text": message,
    }
    ...
    try:
        ...
        print("TELEGRAM ERROR:", str(exc), flush=True)
        return False


def send_startup_test_message() -> None:
    if not app.config.get("SEND_STARTUP_TELEGRAM_TEST"):
        return
    if not telegram_is_configured():
        print("STARTUP TEST: telegram not configured", flush=True)
        return
    ...
    print("APP STARTED - BEFORE TELEGRAM TEST", flush=True)
    send_telegram_notification(startup_message)
    print("APP STARTED - AFTER TELEGRAM TEST", flush=True)
...
def reminder_worker_loop() -> None:
    while True:
        try:
            ...
            print("REMINDER WORKER ERROR:", str(exc), flush=True)
...
def api_test_telegram():
    ...
    return jsonify({"ok": ok, "configured": telegram_is_configured(), "method": request.method})
...
print("APP BOOTSTRAP: database ready", flush=True)
